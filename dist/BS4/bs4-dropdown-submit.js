...
if (dropdowns) {
    $(dropdowns).each(function (x, dropdown) {
        //if contains '.' it will not reachable.
        //make another variable to store id for <input>
        var dropdownName = $(dropdown).attr('name');
        var dropdownNameID = dropdownName.replace('.', '_');
        var ul = $(dropdown).children('.dropdown-menu')[0];
        var btn = $(dropdown).children('.dropdown-toggle')[0];
        $('<span class="inside"></span>').appendTo(btn);
        $('<input type="hidden" id="' + dropdownNameID + '" value="" name="' + dropdownName + '" />').appendTo(dropdown);
